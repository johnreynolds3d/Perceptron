...
void train(int num_training_sets, TrainingSet **training_sets, int num_weights, double *weights,
           double *bias, double *total_error, int num_epochs) {
  assert(training_sets != NULL && weights != NULL && bias != NULL && total_error != NULL);

  puts("I start out by setting my weights and bias to random numbers between -1 and 1:\n");
...
double calc_dot_product_bias(int num_weights, double *weights, double *inputs, double *bias) {
...
int calc_output(int num_weights, double *weights, double input_1, double input_2, double *bias) {
  assert(weights != NULL && bias != NULL);

  double inputs[] = {input_1, input_2};
  double dot_product_bias = calc_dot_product_bias(num_weights, weights, inputs, bias);

  return (dot_product_bias > 0) ? 1 : 0;
}

void update_weights(TrainingSet **training_sets, int num_weights, double *weights, double *bias,
                    double *total_error, int j) {
  assert(training_sets != NULL && weights != NULL && bias != NULL && total_error != NULL);

  double error =
      training_sets[j]->output - calc_output(num_weights, weights, training_sets[j]->inputs[0],
...
void TrainingSet_print(TrainingSet *training_set) {
  assert(training_set != NULL);
  printf("\t[%d %d] %d\n", training_set->inputs[0], training_set->inputs[1], training_set->output);
}
